			...
			<c:when test="${wfaList.size() > 0}">
				<ul>
					<c:forEach var="wfa" items="${wfaList}">
						<li>
						<label>
						<button onclick="toggleModal('${wfa.username}')">회원명 :</button>
						</label>
									${wfa.username}
									<label>신청일자 : </label>
									${wfa.applyDate}
			...
			function alertContents() {
				if (httpRequest.readyState === XMLHttpRequest.DONE) {
					if (httpRequest.status === 200) {
						...
						userDetailViews.innerHTML = "<ul>" + "<li><span>아이디 :"
								...
								+ item.Detail.dictum
								+ "</span></li>"
								+ "<li><input type='button' value='승인허가' onclick=grantApproval('"
								+ item.Detail.username + "')></li>"
						"</ul>";
					} else {
